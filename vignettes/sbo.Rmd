...
* `sbo_kgram_freqs`: A collection of $k$-gram frequency tables, with $k$ up to a given order $N$. These are obtained from a training corpus through the function `kgram_freqs()`.
* `sbo_predictor`: these objects directly store next-word predictiors, allowing for memory compression and fast access. They can be built from `sbo_kgram_freqs` objects through `train_predictor()`.
...
```{r}
(freqs <- kgram_freqs(train, N = 3, dict)) # 'N' is the order of n-grams
```
...
These objects are a "raw" equivalent of a text predictor, and can be created with `sbo_predtable()`:

```{r}
t <- sbo_predtable(freqs)
```
...
### Some details on `sbo_kgram_freqs` and `sbo_predictor` class objects
As the name suggests, objects of class `sbo_kgram_freqs` store the counts for each $k$-gram observed in the training corpus. These are the building block
...
sapply(list(sbo_predtable = t, sbo_kgram_freqs = freqs), size_in_MB)
```
