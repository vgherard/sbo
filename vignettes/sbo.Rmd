...
## Building a text predictor with `sbo`
...
[^1]: This is a small samples of $7·10^4$ entries from the "Tweets" Swiftkey dataset fully available [here](https://www.kaggle.com/crmercado/tweets-blogs-news-swiftkey-dataset-4million).
...
As a concrete example, we test the text-predictor trained in the previous section over the example dataset `sbo::twitter_test`.

```{r}
set.seed(840)
(evaluation <- eval_sbo_predictor(p, sbo::twitter_test)) # test <- sbo::twitter_test
```
