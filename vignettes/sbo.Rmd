---
title: "Text prediction via n-gram Stupid Backoff models"
author: 
...
dict <- get_word_freqs(train) %$% word[1:1000]
head(dict)
```
...
```{r, cache=TRUE}
( freqs <- get_kgram_freqs(train, dict, n = 3) ) # 'n' is the order of n-grams
```
...
1. Replace all punctation including **.**, **?**, **!**, **:**, **;** (any number of any of these) with a single **.**.
...
We can estimate predictive accuracy as follows (the uncertainty in the estimate is approximated by the binomial formula $\sigma = \sqrt{\frac{p(1-p)}{N}}$):
...
Here I list some desired features for the next version of `sbo`:

- Implement a compressed (trie?) data structure for `kgram_freqs` and `sbo_preds` class objects.
- Add a `chunk_size` option in `get_kgram_freqs`, for extracting $k$-gram frequency in chunks (current, fully vectorized, version is too expensive from the memory point of view).
- Add a `preprocessor` and `tokenizer` argument to `kgram_freqs`, to allow for user-defined  preprocessor and tokenizer functions.
- Vectorize `eval_sbo_preds` to allow for rapid evaluation on large test sets (current version employs a slow `for` loop to get its multiple predictions).
