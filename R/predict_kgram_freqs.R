...
#' @md
#
#' @param object a \code{sbo_kgramfreqs} object.
...
################################################################################
predict.sbo_kgramfreqs <- function(object, newdata, lambda = 0.4, ...){
        ...
        newdata %>% 
                get_kgram_prefix(N, dict, EOS) %>%
                `names<-`(paste0("w", 1:(N - 1))) -> newdata
